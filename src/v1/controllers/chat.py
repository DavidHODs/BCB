...
class ChatController:
  ...
  def create_room(self, chat_data: ChatRoomSchema, db: Session = Depends(
  ...
  async def disconnect(self, chat_room_id: str,
                       websocket: WebSocket) -> APIResponse[str] | Response:
    try:
      return await self.chat_service.disconnect(chat_room_id, websocket)
    except AppException as exc:
      return ExceptionHandler.handle_error(exc)

  async def broadcast(self, chat_room_id: str, payload: ChatRoomMessageSchema,
                      db: Session = Depends(get_db)) -> APIResponse[str] | Response:
    try:
